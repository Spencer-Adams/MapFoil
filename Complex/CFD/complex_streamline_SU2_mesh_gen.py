...
class mesh_generation_object(cylinder, vort_panel):
    ...
    def calc_radial_growth_from_deltaD0(self, N, deltaD_0, start_xy=np.array([0.0, 0.0]), end_xy=np.array([1.0, 1.0]), total_distance=None):
        """
        Compute the required radial growth factor `r` such that N points with
        initial spacing deltaD_0 reach from start_xy to end_xy.
        """
        if total_distance is None:
            x_start, y_start = start_xy
            x_end, y_end = end_xy
        ...
        if np.isclose(total_distance, 0.0):
            return 1.0  # Degenerate case: no spacing needed

        if N == 2:
            return total_distance / deltaD_0

        # Use root_scalar to solve for r
        result = root_scalar(
            self.geometric_sum_residual,
            args=(N, deltaD_0, total_distance),
            bracket=[1.00000001, 100.0],
            method='brentq'
        )

        if not result.converged:
            raise RuntimeError("Failed to converge on radial growth factor.")

        return result.root
    
    def geometric_sum_residual(self, r, N, deltaD_0, total_distance):
        """
        Residual function for solving geometric spacing:
        Computes the difference between the geometric sum and the desired total distance.
        """
        if np.isclose(r, 1.0):
            return deltaD_0 * (N - 1) - total_distance
        return deltaD_0 * (1 - r**(N - 1)) / (1 - r) - total_distance
    ...
    def calc_wrapped_stream_or_potential_line(self, start_point: np.array, direction: int, psi_targets=None, func=None, radial_growth=None, deltaD_0=None, forward_stag = True):
        """
        ...
        if psi_targets is not None:
            ...
            # Step outward based on geometric spacing
                        # Optional: allow spacing-first mode
            ...
            if deltaD_0 is not None and forward_stag == True:
                # calculate deltaD_0 which is the spacing between the last two points of upper surface
                deltaD_0 = np.linalg.norm((self.upper_coords[-1, :2] - self.upper_coords[-2, :2])/2) # this is the spacing between the last two points of upper surface. The :2 means we only take the first two coordinates (x and y)
                radial_growth = self.calc_radial_growth_from_deltaD0(self.radial_nodes, deltaD_0, total_distance=total_distance)
            elif deltaD_0 is not None and forward_stag == False:
                # calculate deltaD_0 which is the spacing between the first two points of upper surface
                deltaD_0 = np.linalg.norm((self.upper_coords[1, :2] - self.upper_coords[0, :2])/2) # this is the spacing between the first two points of upper surface
                radial_growth = self.calc_radial_growth_from_deltaD0(self.radial_nodes, deltaD_0, total_distance=total_distance)
            else:
    ...
    def find_correct_ds_for_psi(self, starting_ds, point, direction, target_psi, func=None, tolerance=1e-4, max_iter=10000):
        ...
        return self.solve_unbracketed_psi(point, direction, target_psi, func, guess=starting_ds)

    
    def solve_unbracketed_psi(self, point, direction, target_psi, func, guess=1.0):
        """Employ least-squares to find minimum ds for unbracketed ψ target."""
        def objective(ds):
            return self.psi_error(point, direction, ds[0], self.surface_normal, func, target_psi)
        result = least_squares(objective, x0=[guess], bounds=(1e-10, np.inf))
        if result.success:
            return result.x[0]
        else:
            print("Least squares solver failed for psi target.")
            return guess

    def find_correct_ds(self, starting_ds, point, direction, delta_position, tolerance=1e-4, func=None):
        """
        Find ds such that the change in position matches delta_position using least squares.

        Uses RK4 stepping in the specified direction with `func`, aiming to match the desired arc length.
        """
        if func is None:
            func = self.unit_velocity
        def residual(ds_array):
            ds_val = ds_array[0]
            return self.function_for_bisection_or_newton_method(ds_val, point, direction, delta_position, func=func)
        # Solve for ds using least squares, with bounds to avoid negative steps
        result = least_squares(residual, x0=[starting_ds],bounds=(1e-10, 1e6),xtol=tolerance,ftol=tolerance)
        # Compute error for result.x[0] and starting_ds
        ds_candidate = result.x[0]
        error_candidate = abs(self.function_for_bisection_or_newton_method(ds_candidate, point, direction, delta_position, func=func))
        error_starting = abs(self.function_for_bisection_or_newton_method(starting_ds, point, direction, delta_position, func=func))
        if result.success:
            # Return the ds with the smaller error
            if error_candidate < error_starting:
                return ds_candidate
            else:
                return starting_ds
        else:
            print("Warning: least_squares failed to converge in find_correct_ds.")
            print("Residual:", result.fun[0], "Tolerance:", tolerance)
            # Return the ds with the smaller error even if not successful
            if error_candidate < error_starting:
                return ds_candidate
            else:
                return starting_ds
    ...
    def create_body_mesh(self):
        ...
        self.closest_upper_point = closest_upper_points[sorted_indices[0]] # this is the closest point in the upper surface to the midline x-coordinate
        ...
        # Move stagnation points in the x-direction only
        self.forward_stag[0] -= 1e-12  # Move forward stagnation point left
        # self.forward_stag[1] -= 1e-12      # Move forward stagnation point down
        self.aft_stag[0] += 1e-12  # Move aft stagnation point right
...
if __name__ == "__main__":
    ...
    for D in D_values:
        ...
        if mesh.D <= 0.001:
            ...
            r_values = np.array([mesh.cylinder_radius])
            theta_array = np.array([0.0, np.pi/2, np.pi/1000])
            theta_values = hlp.list_to_range(theta_array)
            appellian_root, xi_eta_vals, appelian_value = mesh.run_appellian_roots(r_values, theta_values, mesh.D)
            print("appellian circulation: ", appellian_root)
            mesh.circulation = appellian_root
        #### the mesh filename includes the zeta_center, D value, angle of attack, radial nodes, radial distance, surface nodes, and radial growth 
        mesh.mesh_filename = f"mesh_zeta0_{mesh.zeta_center.real:.3f}_{mesh.zeta_center.imag:.3f}_D_{mesh.D:.3f}_AoA_{np.rad2deg(mesh.angle_of_attack):.1f}deg_radial_nodes_{mesh.radial_nodes}_surface_nodes_{mesh.surface_nodes}_radial_distance_{mesh.radial_distance}_radial_growth_{mesh.radial_growth}.su2"
        ...
        if mesh.is_generate_mesh and not mesh_file_already_exists:
            ...
            print("Forward stagnation point:", mesh.forward_stag)
            print("Aft stagnation point:", mesh.aft_stag)
            ...
            forward_stag_streamline = mesh.calc_wrapped_stream_or_potential_line(mesh.forward_stag, -1, deltaD_0 = 0.0, forward_stag=True)[1:] # deltaD_0 = 0.0 means we use the spacing between the last two points of the upper surface
            ...
            aft_stag_streamline = mesh.calc_wrapped_stream_or_potential_line(mesh.aft_stag, 1, deltaD_0=0.0, forward_stag =False)[1:] # deltaD_0 = 0.0 means we use the spacing between the last two points of the upper surface
            ...
            #### do the first and last upper potential lines extending from the stagnation points
            forward_stag_upper_potential_lines = mesh.calc_wrapped_stream_or_potential_line(mesh.forward_stag, 1, psi_targets=upper_stream_function_values, func=mesh.unit_normal_velocity, radial_growth=mesh.radial_growth)[1:]  # skip surface point
            aft_stag_upper_potential_lines = mesh.calc_wrapped_stream_or_potential_line(mesh.aft_stag, 1, psi_targets=upper_stream_function_values, func=mesh.unit_normal_velocity, radial_growth=mesh.radial_growth)[1:]  # skip surface point
            for i in tqdm(range(len(mesh.upper_coords)), desc="Calculating Upper Surface Potential Lines", unit="line"):
                if i == 0: # this is associated with the first point of the upper surface (which is the trailing edge)
                    aft_upper_pot_line = mesh.make_N_by_2_an_N_by_three(aft_stag_upper_potential_lines, last_index_upper_midline)
                    last_index_upper_midline = aft_upper_pot_line[-1, 2] 
                    matched_upper_lines.append(aft_upper_pot_line)
                elif i == len(mesh.upper_coords) - 1: # this is associated with the last point of the upper surface (which is the leading edge)
                    forward_upper_pot_line = mesh.make_N_by_2_an_N_by_three(forward_stag_upper_potential_lines, last_index_upper_midline)
                    last_index_upper_midline = forward_upper_pot_line[-1, 2]
                    matched_upper_lines.append(forward_upper_pot_line)
                elif not np.allclose(mesh.upper_coords[i, :2], mesh.closest_upper_point[:2]): # check if the point is not the closest upper point, the :2 means we only take the first two coordinates (x and y)
            ...
            for i in range(len(matched_upper_lines)-1):
                ...
                # make sure the line goes from right to left (in terms of x values)
            ...
            # make sure the line goes from left to right (in terms of x values)
            if line[0][0] > line[1][0]:
                line = line[1], line[0]
            ...
            for i in range(len(leading_edge_upper_potential_lines[-1])-1):
                ...
                #### make sure the line goes from top to bottom (in terms of y values)
            ...
            for i in range(len(trailing_edge_lower_potential_lines[-1])-1):
                ...
                #### make sure the line goes from bottom to top (in terms of y values)
                if line[0][1] > line[1][1]:
                    line = line[1], line[0]
                outer_boundary_lines.append(line)
            #### connect the last point of the trailing edge lower potential line to the last point of the trailing edge streamline
            line = trailing_edge_lower_potential_lines[-1][0], aft_stag_streamline[-1]
            #### make sure the line goes from bottom to top (in terms of y values)
            if line[0][1] > line[1][1]:
                line = line[1], line[0]
